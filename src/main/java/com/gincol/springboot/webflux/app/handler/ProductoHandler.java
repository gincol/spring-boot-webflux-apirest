package com.gincol.springboot.webflux.app.handler;
...
import com.gincol.springboot.webflux.app.services.ProductoService;
...
public class ProductoHandler {
	...
	public Mono<ServerResponse> crearConValidacion(ServerRequest request){
		...
		return producto.flatMap(p -> {
			...
			if(errors.hasErrors()) {
				...
				return service.save(p).flatMap(pdb->ServerResponse.created(URI
						.create("/api/v2/productos/".concat(pdb.getId())))
	...
	public Mono<ServerResponse> crearConFoto(ServerRequest request) {
		...
		Mono<Producto> producto = request.multipartData().map(multipart -> {
			...
			FormFieldPart catetoriaId = (FormFieldPart) multipart.toSingleValueMap().get("categoria.id");
			...
			categoria.setId(catetoriaId.value());
	...
	public Mono<ServerResponse> crear(ServerRequest request){
		...
		.flatMap(p->ServerResponse.created(URI
				.create("/api/v2/productos/".concat(p.getId())))
				.contentType(MediaType.APPLICATION_JSON)
				.body(fromValue(p)));
	}
